...
class TestRuleMatching(unittest.TestCase):
    ...
    def test_ufw_critical(self):
        watcher = RuleWatcher("/var/log/ufw.log", "rules/ufw.yaml", 1, {"host": "localhost", "topic_base": "log"})
    ...
    def test_fail2ban_critical(self):
        watcher = RuleWatcher("/var/log/fail2ban.log", "rules/fail2ban.yaml", 1, {"host": "localhost", "topic_base": "log"})
        line = "2025-07-04 01:00:00,000 fail2ban.actions [1234]: NOTICE  [sshd] Ban 192.168.1.130"
        results = watcher.check_line(line)
        self.assertTrue(any(r[0] == "critical" for r in results))
    def test_murmur_critical(self):
        watcher = RuleWatcher("/var/log/mumble-server/mumble-server.log", "rules/murmur.yaml", 1, {"host": "localhost", "topic_base": "log"})
    ...
    def test_ssh_critical(self):
        watcher = RuleWatcher("/var/log/auth.log", "rules/ssh.yaml", 1, {"host": "localhost", "topic_base": "log"})
